...
describe('pool fetch NUTs', () => {
  ...
  it('should fetch a scratch org from the pool and return valid JSON', () => {
    const result = execCmd<PoolFetchResult>('pool fetch --pool-tag nut-test-pool --json', {
      ensureExitCode: 0,
    }).jsonOutput?.result;

    expect(result).to.have.property('username').that.is.a('string');
    expect(result).to.have.property('orgId').that.is.a('string');
    expect(result).to.have.property('poolTag').that.equals('nut-test-pool');
    expect(result).to.have.property('isDefault').that.is.a('boolean');
  });

  it('should mark the fetched org as assigned', () => {
    const listResult = execCmd<PoolListResult>('pool list --pool-tag nut-test-pool --json', {
      ensureExitCode: 0,
    }).jsonOutput?.result;

    const pool = listResult?.pools.find((p) => p.tag === 'nut-test-pool');
  ...
  it('should produce human-readable output without --json', () => {
    ...
    const output = execCmd('pool fetch --pool-tag nut-test-pool', {
      ensureExitCode: 0,
    }).shellOutput.stdout;

    expect(output).to.include('Fetched scratch org');
    expect(output).to.include('nut-test-pool');
  });
